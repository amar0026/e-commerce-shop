...
/* ─────────────────────────────────────────────
   CONTEXT HOOKS — replace with your real imports:
...
const useWishlist = () => {
  ...
  return {
    isInWishlist:      (id: string) => list.includes(id),
    addToWishlist:     (p: any)     => setList(l => [...l, p.id]),
    removeFromWishlist:(id: string) => setList(l => l.filter(x => x !== id)),
  };
};
const useCart = () => ({ addToCart: (p: any) => console.log('cart +', p.title) });
...
const SIZE_DATA: Record<SizeTab, { cols: string[]; rows: { size: string; chest: string; waist: string; hip: string; length: string }[] }> = {
  Men: {
    ...
    rows: [
      { size:'XS',  chest:'34–35', waist:'27–28', hip:'34–35', length:'28' },
      { size:'S',   chest:'36–37', waist:'29–30', hip:'36–37', length:'29' },
      { size:'M',   chest:'38–39', waist:'31–32', hip:'38–39', length:'30' },
      { size:'L',   chest:'40–41', waist:'33–34', hip:'40–41', length:'31' },
      { size:'XL',  chest:'42–43', waist:'35–36', hip:'42–43', length:'32' },
  ...
  Women: {
    ...
    rows: [
      { size:'XS',  chest:'32–33', waist:'25–26', hip:'34–35', length:'37' },
      { size:'S',   chest:'34–35', waist:'27–28', hip:'36–37', length:'38' },
      { size:'M',   chest:'36–37', waist:'29–30', hip:'38–39', length:'39' },
      { size:'L',   chest:'38–40', waist:'31–33', hip:'40–42', length:'40' },
      { size:'XL',  chest:'41–43', waist:'34–36', hip:'43–45', length:'41' },
      { size:'XXL', chest:'44–46', waist:'37–39', hip:'46–48', length:'42' },
    ],
  },
  Kids: {
    cols: ['Size', 'Chest (in)', 'Waist (in)', 'Hip (in)', 'Height (cm)'],
    rows: [
      { size:'2Y',  chest:'21',    waist:'20',    hip:'22',    length:'92'  },
      { size:'4Y',  chest:'23',    waist:'21.5',  hip:'24',    length:'104' },
      { size:'6Y',  chest:'25',    waist:'23',    hip:'26',    length:'116' },
      { size:'8Y',  chest:'27',    waist:'24.5',  hip:'28',    length:'128' },
      { size:'10Y', chest:'28.5',  waist:'25.5',  hip:'30',    length:'140' },
      { size:'12Y', chest:'30',    waist:'27',    hip:'32',    length:'152' },
    ],
  },
};
      ...
      >
        ...
        <div className="relative px-6 py-5 border-b border-black/6 bg-gradient-to-r from-orange-50 to-white overflow-hidden">
          ...
          <div className="relative z-10 flex items-center justify-between">
            ...
            <button
              onClick={onClose}
              className="w-8 h-8 rounded-xl flex items-center justify-center text-gray-400 hover:bg-red-50 hover:text-red-500 cursor-pointer bg-transparent border-none transition-all"
            >
              <X size={15} />
            </button>
          </div>
            ...
            {SIZE_TABS.map(tab => (
              <button
                ...
                className={`text-[11px] font-bold tracking-[.12em] uppercase px-5 py-2 rounded-2xl cursor-pointer border-none transition-all duration-200
                  ${activeTab === tab
                    ? 'bg-orange-500 text-white shadow-[0_4px_14px_rgba(249,115,22,.35)]'
                    : 'bg-white border border-black/8 text-gray-500 hover:border-orange-300 hover:text-orange-500'
                  }`}
              >
        ...
        <div className="overflow-y-auto" style={{ maxHeight: 'calc(90vh - 160px)' }}>
          ...
          <div className="p-5">
              ...
              <table className="w-full text-left text-[12px]" style={{ fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
                <thead>
                  <tr className="bg-orange-50 border-b border-orange-100">
                    {data.cols.map((col, i) => (
                      <th key={col}
                        className={`px-4 py-3 font-black text-[10px] tracking-[.16em] uppercase whitespace-nowrap
                ...
                <tbody>
                  {data.rows.map((row, i) => (
                    <tr key={row.size}
                      className={`border-b border-black/4 last:border-0 hover:bg-orange-50/50 transition-colors`}
                      style={{ animation: `rowFadeIn .35s cubic-bezier(.16,1,.3,1) ${i * 0.04}s both` }}
                    >
                      <td className="px-4 py-3">
                        <span className="inline-flex items-center justify-center w-9 h-8 bg-orange-500 text-white font-black text-[11px] rounded-xl">
                          {row.size}
                        </span>
                      </td>
            ...
            <p className="text-[10px] text-gray-400 mt-2 text-center">
              All measurements are in inches unless stated otherwise. Sizes may vary slightly by style.
            </p>
          </div>
          ...
          <div className="px-5 pb-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {HOW_TO_MEASURE.map((tip, i) => (
                <div key={tip.label}
                  className="flex items-start gap-3 p-3.5 bg-orange-50/60 border border-orange-100 rounded-2xl"
                  style={{ animation: `rowFadeIn .4s cubic-bezier(.16,1,.3,1) ${i * 0.06 + 0.2}s both` }}
                >
                  <div className="w-7 h-7 bg-orange-500 rounded-xl flex items-center justify-center shrink-0 font-black text-white text-[10px]">
                    {i + 1}
                  </div>
                  <div>
        ...
        <div className="px-5 py-4 border-t border-black/5 bg-gray-50/40 flex items-center justify-between gap-3 flex-wrap">
          <p className="text-[11px] text-gray-400">
            💡 <strong className="text-[#111]">Tip:</strong> When between sizes, size up for a relaxed fit.
          </p>
          <button
            onClick={onClose}
...
const PRODUCTS: GarmentProduct[] = [
  { id:'1', title:'Cotton Blend Co-ord Set',    brand:'Aurelia',     price:1299, originalPrice:1899,
    ...
    rating:4.5, reviewCount:'128', status:'New',        category:'Women',
    description:'', fabric:'100% Cotton',  sizes:['XS','S','M','L','XL'],    badge:'New Arrival', badgeColor:'#16a34a', discount:32 },
  ...
  { id:'2', title:'Embroidered Anarkali Kurta', brand:'Biba',        price:2150, originalPrice:3200,
    ...
    rating:4,   reviewCount:'75',  status:'Bestseller', category:'Ethnic',
    description:'', fabric:'Georgette',    sizes:['S','M','L','XL','XXL'],   badge:'Sale',        badgeColor:'#f97316', discount:33 },
  ...
  { id:'3', title:'Slim Fit Stretch Chinos',    brand:'FabIndia',    price:899,  originalPrice:1299,
    ...
    rating:4.5, reviewCount:'99',  status:'Bestseller', category:'Men',
    description:'', fabric:'Cotton Blend', sizes:['30','32','34','36','38'], badge:'Bestseller',  badgeColor:'#111111', discount:31 },
  ...
  { id:'4', title:'Tropical Rayon Camp Shirt',  brand:'Manyavar',    price:749,  originalPrice:1100,
    ...
    rating:4.5, reviewCount:'214', status:'Trending',   category:'Men',
    description:'', fabric:'Rayon',        sizes:['S','M','L','XL'],         badge:'Trending',    badgeColor:'#ea580c', discount:32 },

  { id:'5', title:'Kids Dungaree Playsuit',     brand:'H&M Kids',    price:599,  originalPrice:899,
    image:'https://res.cloudinary.com/dquki4xol/image/upload/v1773296148/images_26_wxn2do.jpg',
    rating:4.8, reviewCount:'61',  status:'New',        category:'Kids',
    description:'', fabric:'Denim Cotton', sizes:['2Y','4Y','6Y','8Y'],      badge:'New Arrival', badgeColor:'#16a34a', discount:33 },

  { id:'6', title:'Palazzo Sharara Set',        brand:'W for Woman', price:1799, originalPrice:2499,
    ...
    rating:4.3, reviewCount:'88',  status:'Bestseller', category:'Ethnic',
    description:'', fabric:'Silk Blend',   sizes:['S','M','L','XL'],         badge:'Sale',        badgeColor:'#f97316', discount:28 },
  ...
  { id:'7', title:'Flared Denim Jeans',         brand:'Levis',       price:1499, originalPrice:2299,
    ...
    rating:4.6, reviewCount:'302', status:'Trending',   category:'Women',
    description:'', fabric:'98% Denim',    sizes:['26','28','30','32'],       badge:'Trending',    badgeColor:'#ea580c', discount:35 },

  { id:'8', title:'Bandhgala Nehru Jacket',     brand:'Manyavar',    price:3299, originalPrice:4999,
    image:'https://res.cloudinary.com/dquki4xol/image/upload/v1773295573/4_2ab5d4e8-2cc9-4134-83a2-e8a061395274_w6xpfq.webp',
    rating:4.7, reviewCount:'47',  status:'New',        category:'Western',
    description:'', fabric:'Art Silk',     sizes:['S','M','L','XL','XXL'],   badge:'New Arrival', badgeColor:'#16a34a', discount:34 },
];
...
const ShopCollection: React.FC = () => {
  ...
  const [showSizeGuide, setShowSizeGuide] = useState(false); // ← SIZE GUIDE STATE
        ...
        <div className="relative z-10 max-w-7xl mx-auto">
          ...
          <div style={fade(0.06)} className="flex items-end justify-between gap-4 flex-wrap mb-7">
            <div>
              ...
              <h2 className="font-extrabold text-[#111] leading-none tracking-tight"
                style={{ fontSize:'clamp(1.9rem,4vw,3rem)' }}>
                Our Latest{' '}
                <em className="italic font-semibold text-orange-500" style={{ fontStyle:'italic' }}>Collection</em>
              </h2>
              ...
              return (
                <button key={f} onClick={() => setActiveFilter(f)}
                  className={`inline-flex items-center gap-1.5 text-[11px] font-semibold tracking-[.1em] uppercase px-4 py-[7px] rounded-2xl border transition-all duration-200 cursor-pointer
                    ${isActive ? 'bg-orange-500 border-orange-500 text-white shadow-[0_4px_14px_rgba(249,115,22,.3)]'
                  ...
                  <span>{f}</span>
                  <span className={`inline-flex items-center justify-center w-4 h-4 rounded-full text-[9px] font-bold ${isActive ? 'bg-white/25 text-white' : 'bg-orange-100 text-orange-600'}`}>
                    {count}
                  </span>
                </button>
              );
            ...
            {visibleProducts.map((product, i) => (
              <div key={product.id}
                ...
                <div className="relative flex items-center justify-center overflow-hidden"
                  style={{ height:'clamp(190px,24vw,250px)', background:'#fff4ee' }}>
          ...
          <div style={fade(0.46)} className="flex items-center justify-center gap-3 flex-wrap">
            ...
            {/* ← SIZE GUIDE button — opens the modal */}
            <button
